% lexer from https://github.com/minijackson/paddle
Nonterminals
	schema
...
% WOId type
real_noidlen -> noidlen : '$1'.
real_noidlen -> numericoid : '$1'.
real_noidlen -> woid : '$1'.
...
equality             -> 'EQUALITY' woid         : [{equality, '$2'}].
ordering             -> 'ORDERING' woid         : [{ordering, '$2'}].
substr               -> 'SUBSTR' woid           : [].
syntax               -> 'SYNTAX' real_noidlen   : [{syntax, '$2'}].
single_value         -> 'SINGLE-VALUE'          : [{single_value, 'true'}].
